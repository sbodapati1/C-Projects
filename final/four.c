...
WordCountTable *sum_word_count_tables(WordCountTable *table_a,
  ...
  if(table_a->num_buckets >= table_b->num_buckets){
  	out = build_word_count_table(table_a->num_buckets);
  }
  else{
  ...
  for(size_t i = 0; i < sizeof(out); i++){
	  if(table_a->buckets[i] != NULL){
	    char *words = table_a->buckets[i]->word;
	    size_t counta = get_word_count(words, table_a);
	    size_t countb = get_word_count(words, table_b);
	    size_t totalcount = counta + countb;
	    set_word_count(words, totalcount, out);
	    }
  }  
  for(size_t i = 0; i < sizeof(table_b); i++){
	  if(table_b->buckets[i] != NULL){
		  char *words = table_b->buckets[i]->word;
		  size_t countb = get_word_count(words, table_b);
		  if(get_word_count(words, out) == 0){
			  set_word_count(words, countb, out);
		  }
	  }
  }
  return out;
