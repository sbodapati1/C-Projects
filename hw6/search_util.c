...
int score_word(char *word, int *letter_scores) {
  ...
  for(unsigned long l = 0; l < 5; l++){
          for(unsigned long j = l + 1; words[j] != '\0'; j++){
                if(words[j] == words[l]){
                        words[j] = '\0';
                }
                }
  }
  for(unsigned long h = 3; h < 5; h++){
          if(words[h] == words[h+1]){
                  words[h+1] = '\0';
         }
  }
  for(unsigned long m = 0; m < 6; m++){
       for(unsigned long k = 0; k < 26; k++){
                if(letters[k] == words[m]){
		    count += letter_scores[k];
                }       
        }       
  }
  return count;
...
size_t filter_vocabulary_yellow(char letter, int position, char **vocabulary,
  ...
  // TODO(you): implement this functioin!
  int count = 0;
  char *word;
  for(size_t i = 0; i < num_words; i++){
		  word = vocabulary[i];
