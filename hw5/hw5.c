...
bool score_guess(char *secret, char *guess, char *result) {
  ...
  if(strcmp(guess, secret) == 0){
	  ...
	  for(unsigned long i=0; i < 6; i++){
		  ...
		  if(strchr(&secret[i], guess[i]) == strchr(&guess[i], secret[i])){
			  result[i] = 'g';
		  }
		  if(strcmp(&guess[i], &secret[j]) == 0){
			  result[i] = 'y';
		  }
		  else{
			  result[i] = 'x';
...
bool valid_guess(char *guess, char **vocabulary, size_t num_words) {
  ...
  for(unsigned long i=0; i < num_words; i++){
     if(vocabulary[i] != NULL && guess != NULL){ 
         //printf("wordsd, %c", *vocabulary[i]);	      
...
char **load_vocabulary(char *filename, size_t *num_words) {
  ...
  // TODO(you): finish this function
  *num_words = 2309;
  out = (char**)malloc((*num_words) * sizeof *out);
  FILE* vocab = fopen(filename, "r");
  char buf[2310];
  char *line;
  while(fgets(buf, 2310, vocab) != NULL){
	  for(int i=0; i < 2310; i++){
	      if(fgets(buf, i, vocab) != NULL){
                  line = fgets(buf, i, vocab);
		  line[strcspn(line, "\n")] = 0;
		  out[i] = strndup(line, 7);
	      	  if(sizeof *out == *num_words){
                  	out = (char**)realloc(out, 10 * *num_words);
  	      }
	      }
	  }	      
  }
